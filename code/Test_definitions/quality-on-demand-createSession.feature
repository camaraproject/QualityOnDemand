Feature: CAMARA Quality On Demand API, 1.2.0-alpha.1 - Operation createSession
  ...
  Background: Common createSession setup
    ...
    And the resource "/quality-on-demand/v1alpha1/sessions"
  ...
  @quality_on_demand_createSession_02_2_event_received
  Scenario: Event is received when a session status changes after using sink
