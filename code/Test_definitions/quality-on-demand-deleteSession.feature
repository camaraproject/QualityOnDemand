Feature: CAMARA Quality On Demand API, v1.2.0-rc.3 - Operation deleteSession
  ...
  Background: Common deleteSession setup
    ...
    And the resource "/quality-on-demand/v1rc3/sessions/{sessionId}"
  ...
  @quality_on_demand_deleteSession_400.2_invalid_x-correlator
  ...
  @quality_on_demand_deleteSession_429.1_too_many_requests
  ...
  Scenario: Request is rejected due to threshold policy
    Given a valid request for "deleteSession"
    And the header "Authorization" is set to a valid access token
